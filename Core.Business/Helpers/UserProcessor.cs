...
namespace Core.Business
{
    public class UserProcessor : ProcessorBase<IUser>
    {
        ...
        public async Task<IProcessorResult<IUser>> Login(string username, string password)
        {
            IProcessorResult<IUser> result;
            try
            {
                var users = await DataHelper.FetchQuery(string.Format(@"(Username eq '{0}') and(Password eq '{1}')", username, password));
                if (users.Count == 1)
                {
        ...
        public async override Task<IValidationResult> Validate()
        {
